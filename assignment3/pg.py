...
class PG(object):
    ...
    def add_placeholders_op(self):
        ...
        self.observation_placeholder = tf.placeholder(
            dtype=tf.float32, shape=[None, self.observation_dim])
        if self.discrete:
    ...
    def build_policy_network_op(self, scope="policy_network"):
        ...
        if self.discrete:
            ...
            self.sampled_action = tf.squeeze(tf.multinomial(
            ...
            action_logits = build_mlp(
                ...
                n_layers=self.config.n_layers,
                size=self.config.layer_size,
                output_activation=self.config.activation  # or None ??????
            )
            ...
            self.sampled_action = tf.random_normal(
                shape=tf.shape(tf.shape(action_logits)),
                mean=action_logits,
                stddev=tf.exp(log_std))
            # TODO: how to use MultivariateNormalDiag ???
            self.logprob = tf.contrib.distributions.MultivariateNormalDiag(
                loc=action_logits,
                scale_diag=tf.exp(log_std)).log_prob(self.action_placeholder)
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    def add_baseline_op(self, scope="baseline"):
        ...
        self.baseline_target_placeholder = tf.placeholder(
            dtype=tf.float32, shape=[None])
        self.update_baseline_op = tf.train.AdamOptimizer(self.lr).minimize(tf.losses.mean_squared_error(
...
def get_returns(self, paths):
    ...
    for path in paths:
        ...
        returns = [np.sum(
            np.power(
                self.config.gamma,
                np.arange(max_step - t)) * rewards[t:]) for t in range(max_step)]
        #######################################################
        #########          END YOUR CODE.          ############
